...
| Key      | Value Type | Required | Description                                                    |
| -------- | ---------- | -------- | -------------------------------------------------------------- |
| time_rec | uint64     |          | 想要拉取的充电交易数据产生的时间，如果为空表示想要获取最新数据 |
...
服务端发起，要求负控设备启停或控制 P / Q 的时序曲线（P / Q两类控制互斥，不能同时下发）。
...
| p_set      | float32 array | *        | 有功功率设定值（kW），数组长度和 times 数组一致（Y轴）         |
| q_set      | float32 array | *        | 无功功率设定值（kVar），数组长度和 times 数组一致（Y轴）       |
...
服务端发起，要求负控设备启停或控制充电价格或总功率 P / Q 的时序曲线。
