...
服务端即后端，负责接收并记录设备传递回来的信息，发送控制命令管理设备的运行状态。
服务端只有一个固定的 IP，监听指定端口。

## 客户端

客户端即终端设备，负责收集物理机器的运行数据并定时汇报给平台，当平台发送控制命令时客户端应当控制物理设备执行。
设备需要主动连接到服务端的指定 IP 和指定端口。

设备包括两种：非直连设备和网关设备。
与物理机器直接相连的设备是非直连设备，可以获取物理机器的信息并管理其运行状态。
但是非直连设备不具备直接连接到互联网的能力，它必须通过串口或以太网等方式与网关设备连接，经过网关来与服务端交互。

**注意**：经过精简，直连设备相关内容已经被删除。
...
客户端在连接到服务端后，首先应当发起一个特殊的登录命令，发送自身的客户端标识符。
服务端需要验证该标识符是否合法，并将验证结果包含在返回的响应中。

如果客户端为首次连接，则在登录前应当先发起注册命令，服务端返回的响应中会包含客户端标识符，客户端应当将该标识符持久化以用于登录。
